...
cealign tmalign}
    ...
    def myalign(alimethod):
        ...
        start = time.time()
        cmd.do('%s mobile=%s in %s, target=%s' % (
            alimethod, newmobile, mobile, target))
        if not quiet:
            print('Finished: %s (%.2f sec)' % (alimethod, time.time() - start))
...
def tmalign(mobile, target, args='', exe='TMalign', ter=0, transform=1,
            object=None, quiet=0):
    """
DESCRIPTION
...
tmscore, mmalign
    ...
    args = [exe, mobile_filename, target_filename, '-m',
            matrix_filename] + args.split()
    ...
    if int(transform):
        cmd.transform_selection('byobject (%s)' % mobile, matrix,
                                homogenous=1)
    ...
    if object is not None:
        ...
        cmd.iterate(mobile_ca_sele,
                    'mobile_idx.append("%s`%d" % (model, index))', space=space)
        cmd.iterate(target_ca_sele,
                    'target_idx.append("%s`%d" % (model, index))', space=space)
...
def mmalign(mobile, target, args='', exe='MMalign', ter=0, transform=1,
            quiet=0):
    """
DESCRIPTION
